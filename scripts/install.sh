...
# SpectraBox Direct Installation Script
# Simple one-liner installation for Raspberry Pi
...
log_info "Starting SpectraBox installation..."
...
log_info "Installing for user: $PI_USER"
...
apt-get install -y --no-install-recommends \
    ca-certificates curl wget git jq xdg-utils xdotool unclutter \
    libnss3 libatk1.0-0 libxss1 libasound2 libgtk-3-0 \
    libgdk-pixbuf2.0-0 libxcomposite1 libxcursor1 libxdamage1 \
    libxrandr2 libgbm1 libxkbcommon0 libatspi2.0-0 \
    alsa-utils openssl pulseaudio pulseaudio-utils
...
BROWSER_BIN=$(command -v chromium || command -v chromium-browser || command -v firefox-esr)
